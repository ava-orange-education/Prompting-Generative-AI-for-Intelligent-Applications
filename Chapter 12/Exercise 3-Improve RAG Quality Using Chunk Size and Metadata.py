...
chunks_large = create_overlapping_chunks(notes, size=80, overlap=20)
chunks_small = create_overlapping_chunks(notes, size=40, overlap=10)

#-------------------------------------------------#

client = chromadb.Client()
collection = client.create_collection("notes_with_meta")
...
collection.add(
    ids=[f"id_{i}" for i in range(len(chunks_small))],
    documents=chunks_small,
    embeddings=embeddings,
    metadatas=[{"topic": "cell_division"} for _ in chunks_small]
)
...
filtered_chunks = results["documents"]
